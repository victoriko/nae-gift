...
const ReceiveBox: React.FC<GiftListData> = ({ receivedItem }) => {
  ...
  const verify = async () => {
    try {
      ...
      const targetVc = await queried?.data?.find(
        (element: QueryCredentialsRequestResult) => element?.data?.credentialSubject?.giftID === receivedItem.id,
      );
  ...
  const receiveGift = async () => {
    try {
      ...
      if (successRes.data.success) {
        ...
        alert("You have declined the issuance of the digital voucher. Approval is required to receive the gift.");
      }
    } catch (error) {
  ...
  const fulfilledConfirmCheck = () => {
    if (window.confirm("Once confirmed, it cannot be cancelled. Would you like to confirm receipt?")) {
      confirm();
  ...
  return (
    <>
      ...
      >
        ...
        {(receivedItem.state === "issued" || receivedItem.state === "active" || receivedItem.state === "fulfilled") && (
          <div>
            {loading ? (
              <div
                className={cn(
              ...
              <div>
                {btnState === "issued" ? (
                  <Button onClick={issuedConfirmCheck} variant="sendBtn1" size="dd" label="Use Gift" />
                ) : btnState === "active" ? (
                  <Button onClick={activeConfirmCheck} variant="basicBtn" size="dd" label="Receive Gift" />
                ) : btnState === "fulfilled" ? (
                  <Button onClick={fulfilledConfirmCheck} variant="sendBtn1" size="dd" label="Confirm Receipt" />
                ) : null}
              </div>
            )}
          </div>
