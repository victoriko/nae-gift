...
import { Product } from "../../app/gift/page";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className=" mobile:text-sm">"The received gift can be viewed on desktop."</span>
        <span className="tablet:border tablet:flex-1"></span>
      </div>
      <div className="w-1/2 px-8 note:px-3 tablet:mx-auto tablet:w-full mobile:w-full">
        ...
        >
          <div className={cn("flex justify-between items-center ", "tablet:py-3")}>
            <h2 className="text-xl mobile:text-base">🎁 Gift history sent</h2>
            <div className="flex flex-row gap-5 mobile:hidden">
              <Button variant="basicBtn2" size="md" label="Lates" onClick={() => payOrderChange("desc")} />
              <Button variant="basicBtn2" size="md" label="Past" onClick={() => payOrderChange("asc")} />
            </div>
          </div>
          <div className="flex flex-col gap-5 py-5">
            {payProducts && payProducts.length > 0 ? (
              payProducts.map((payItem, payIndex) => <PayBox key={payIndex} payItem={payItem} />)
            ) : (
      ...
      <div className="w-1/2 px-8 note:px-3 tablet:hidden mobile:hidden">
        <div className={cn("w-full bg-slate-50 rounded-md p-5 shadow-md h-[700px]  ")}>
          <div className="flex justify-between items-center tablet:flex-col tablet:gap-5">
            ...
            <div className="flex flex-row gap-5">
              <Button variant="basicBtn2" size="md" label="Latest" onClick={() => receiveOrderChange("desc")} />
              <Button variant="basicBtn2" size="md" label="Past" onClick={() => receiveOrderChange("asc")} />
            </div>
          </div>
          <div className="flex flex-col gap-5 py-5">
            {receiveProducts && receiveProducts.length > 0 ? (
              ...
              <p className=" tablet:text-center">
                {receiveProducts.length === 0 ? "There are no received gifts." : ""}
              </p>
            )}
          </div>
        </div>
